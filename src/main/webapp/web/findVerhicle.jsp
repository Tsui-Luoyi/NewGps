...
<h4>查询车辆:</h4>
	...
	<script>
		...
		function ajaxPaging(){
			...
			success:function(data){
				...
				for(var i=0;i<data.results.length;i++){
					...
					var td3=$("<td><a href='javascript:void(0);' title='修改绑定终端' target='_blank' onClick='changeThisTerminal(this)' data-name='"+data.results[i].verhiclenum+"' data-id='"
							+data.results[i].id+"' class='down btn btn-default btn-xs changeBind'><img src='images/change.png'/></a>"+data.results[i].terminalnum+"</td>");
					var td4=$("<td>"+data.results[i].creattime+"</td>");
					var td5=$("<td>"+data.results[i].verhiclegroup+"</td>");
					var td6=$("<td><a href='javascript:void(0);' title='修改车辆信息' onClick='changeThisVerhicle(this)' data-name='"+data.results[i].verhiclenum+"' data-id='"
							+data.results[i].id+"' class='down btn btn-default btn-xs'>修改</a>"
							+"<a href='javascript:void(0);' onClick='stopThisVerhicle(this)' data-name='"+data.results[i].verhiclenum+"' data-id='"
							+data.results[i].id+"' class='down btn btn-default btn-xs'>停用</a>"
							+"<a href='javascript:void(0);' onClick='deleteThisVerhicle(this)' data-name='"+data.results[i].verhiclenum+"' data-id='"
		...
		//删除函数
		function deleteThisVerhicle(obj){
		...
		/*  修改车辆信息*/
		function changeThisVerhicle(obj){
			alert("修改车辆信息");
		}
		function changeThisTerminal(obj){
			alert("修改终端信息");
			$(obj).attr("href","http://www.baidu.com");
		}
