...
window.onload = function() {
	...
	map.addEventListener("mousemove", function(ev) {
		...
		if (flagRound) {
			...
			roundDis = map.getDistance(roundPoint, new BMap.Point(ev.point.lng,
					ev.point.lat));
		...
		if (flagRect) {
			...
			rect.setPositionAt(0, new BMap.Point(rectStartPoint.lng,
					rectStartPoint.lat));
			rect.setPositionAt(1, new BMap.Point(rectEndPoint.lng,
					rectStartPoint.lat));
			rect.setPositionAt(2, new BMap.Point(rectEndPoint.lng,
					rectEndPoint.lat));
			rect.setPositionAt(3, new BMap.Point(rectStartPoint.lng,
					rectEndPoint.lat))
			map.addOverlay(rect);
		}
		;
			...
			.addEventListener(
					...
					function(e) {
						if (dblFlag && addFenceFlag) {
							...
							flagPolygon = false;
							dblFlag = false;
							$(".mengBan,#findVerhicalResult").css("display",
							"block");
						...
						"columnDefs":[{
								...
								"render" : function(data,type,row) {		
												var html="<a target='_blank' href='checkTrail.jsp?varhicalId="+row.id +"' class='delete btn btn-default btn-xs'><i class='fa fa-times'></i> 查看轨迹</a>"
												return html;
											}
										},{
											//定义操作列,
